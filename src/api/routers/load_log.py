...
def load_log(  # session: SessionInfo = Depends(get_session),
    ...
    if Configuration.enable_load_local_path:
        try:
            ...
            logging.info("load_log_from_path start session=" + str(session_id))
            ...
            session_manager.load_log_from_db(tenant_id, log_id, parameters={
                constants.PARAMETER_CONSTANT_CASEID_KEY: 'case:concept:name',
                constants.PARAMETER_CONSTANT_ACTIVITY_KEY: 'concept:name'
            })

            logging.info(
                "load_log_from_path complete session=" + str(session_id) + " user=" + str(account_name))
