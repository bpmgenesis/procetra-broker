...
# from api.handlers.xes.xes import XesHandler
from api.log_manager.interface.log_manager import LogHandler
...
class BasicLogSessionHandler(LogHandler):
    ...
    def get_handler_for_process_and_session(self, process, session, parameters=None) -> ParquetHandler:
        """
    ...
    def load_log_from_db(self, org_name: str, log_id: str, parameters=None):
        """
        Loads an event log inside the known handlers
        Parameters
        ------------
        ...
        parameters
            Possible parameters
        """
        if log_id not in self.handlers:
            handler = ParquetHandler()
            self.handlers[log_id] = handler;
            handler.build_from_org_db(org_name, log_id, parameters=parameters)
